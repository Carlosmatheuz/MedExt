...
const TopicCard: React.FC<TopicCardProps> = ({ topic, studyData, onToggleStudy, onToggleRevision }) => {
  ...
  return (
    <div className={`group relative bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col ${isStudied ? 'bg-gray-50/50 dark:bg-gray-900/50' : ''}`}>
      ...
      <div className="p-4 pl-6 flex-1">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
             ...
             <h3 className={`font-semibold text-gray-900 dark:text-gray-100 leading-tight text-lg mb-2 ${isStudied ? 'text-gray-600 dark:text-gray-400' : ''}`}>
               {topic.title}
             </h3>
          ...
          <button 
            ...
            className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all ${
              isStudied 
                ...
                : 'bg-gray-100 dark:bg-gray-700 text-gray-300 dark:text-gray-500 hover:text-gray-400 dark:hover:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600'
            }`}
      ...
      {isStudied && (
        <div className="bg-gray-50/50 dark:bg-gray-900/50 p-4 pl-6 pt-0">
          {renderRevisionInfo()}
        </div>
      )}
    </div>
